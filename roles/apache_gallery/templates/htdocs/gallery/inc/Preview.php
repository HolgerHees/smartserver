...
class Preview {
    ...
    public static function check($original_file)
    {
        ...
        if( Preview::$cache_map == null )
        {
            Preview::$cache_map = scandir(CACHE_DIRECTORY . $camera_name . "/");
        }
        ...
        if( !in_array($small_cache_name, Preview::$cache_map ) || !in_array($medium_cache_name, Preview::$cache_map ) || !in_array($org_cache_name, Preview::$cache_map ) )
        {
            Preview::processFile($original_file, $small_cache_name, $medium_cache_name, $org_cache_name);

            if( !in_array($small_cache_name, Preview::$cache_map ) ) array_push(Preview::$cache_map,$small_cache_name);
            if( !in_array($medium_cache_name, Preview::$cache_map ) ) array_push(Preview::$cache_map,$medium_cache_name);
            if( !in_array($org_cache_name, Preview::$cache_map ) ) array_push(Preview::$cache_map,$org_cache_name);
        }

        return array($small_cache_name, $medium_cache_name, $org_cache_name);
    }

    private static function processFile($original_file, $small_cache_name, $medium_cache_name, $org_cache_name)
    {
        $path_parts = pathinfo($original_file);
        ...
        $fp = fopen( $org_cache_path.".lock", "w");
        if( flock($fp, LOCK_EX) )
        {
