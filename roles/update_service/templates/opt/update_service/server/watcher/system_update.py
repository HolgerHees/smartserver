...
class SystemUpdateWatcher(watcher.Watcher): 
    ...
    def initSystemState(self,shouldRetry):
        ...
        if len(_states) == 0:
            self.states = {}
            self.system_update_last_modified = self.smartserver_changes_last_modified = self.smartserver_pull = time.time()
            self.system_updates_hash = self.smartserver_changes_hash = ""
            return
