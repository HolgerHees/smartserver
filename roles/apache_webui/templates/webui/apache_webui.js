...
{% set device_url_scheme=(device.config.webui.scheme if 'config' in device and 'webui' in device.config and 'scheme' in device.config.webui else ("https")) %}
...
subGroup.addUrl('network_device_{{device["host"]}}', ['admin'], '{{device_url_scheme}}://{{device["host"]}}', { 'order': {{device_order}}, 'title': '{{device_name}}', 'info': '{{device_info}}', 'icon': '{{device_icon}}', 'target': '_blank' });
{% endfor %}
