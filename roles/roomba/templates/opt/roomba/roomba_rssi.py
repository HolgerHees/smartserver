...
class Roomba(object):
    ...
    def enable_map(self, enable=False, mapSize="(800,1500,0,0,0,0)",
        ...
        if self.drawmap:
            ...
            self.rssiMap = rssiMap
            self.rssiMapGoodSignal = rssiMapGoodSignal
            self.rssiMapBadSignal = rssiMapBadSignal
    ...
    def decode_topics(self, state, prefix=None):
        ...
        for k, v in six.iteritems(state):
            if isinstance(v, dict):
                ...
                elif k == "pose_point_x": #x and y are reversed...
                    self.co_ords["y"] = v
                elif k == "pose_point_y":
                    self.co_ords["x"] = v
                elif k == "bin_full":
                    self.bin_full = v
                elif k == "cleanMissionStatus_error":
                    try:
                ...
                elif k == "cleanMissionStatus_phase":
                    self.previous_cleanMissionStatus_phase = \
                        self.cleanMissionStatus_phase
    ...
    def draw_vacuum_lines(self, image, old_x_y, x_y, color):
        '''
        ...
        if x_y != old_x_y:
            ...
            lines.line([old_x_y, x_y], fill=color,
        ...
        arcbox = [x_y[0]-self.roomba_icon.size[0] // 4,
                  ...
                  x_y[1]+self.roomba_icon.size[0] // 4]
        lines.ellipse(arcbox, fill=color)
    ...
    def render_map(self, new_co_ords, old_co_ords):
        ...
        if  self.current_state == self.states["charge"]:
            ...
            if self.rssiMap:
                self.base_rssi = Image.new('RGBA', self.base.size, self.transparent)
        ...
        #draw lines
        self.draw_vacuum_lines(self.base, old_x_y, x_y, self.fillColor)
        
        if self.rssiMap:
            
            calculation_base = self.rssi
            if calculation_base > self.rssiMapGoodSignal:
                calculation_base = self.rssiMapGoodSignal
            elif calculation_base < self.rssiMapBadSignal:
                calculation_base = self.rssiMapBadSignal
            
            diff = ( self.rssiMapBadSignal * -1 ) - ( self.rssiMapGoodSignal * -1 )
            
            calculation_base = calculation_base + ( self.rssiMapBadSignal * -1 )  # now should be diff .. 0
            factor = calculation_base / diff # 1 .. 0
            
            #print("%s %s %s %s " % ( self.rssi, diff, calculation_base, factor ))
            
            green = int( 255 * factor )
            red = int( 255 * ( ( factor * -1 ) + 1) )
            
            self.draw_vacuum_lines(self.base_rssi, old_x_y, x_y, (red,green,0,255) )
        ...
        if self.rssiMap:
            rssi_rotated = self.base_rssi.rotate(180 + self.angle, expand=True).\
            resize(self.base.size)
            self.save_rssi_map(rssi_rotated)
        if draw_final:
            self.show_final_map = True  # prevent re-drawing of map until reset

    def save_rssi_map(self, map):
        map.save(self.mapPath + '/' + self.roombaName +
                              'map_rssi.png', "PNG")
